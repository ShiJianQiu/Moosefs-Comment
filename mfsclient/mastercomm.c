...
typedef struct _threc {
	...
	pthread_cond_t cond;//条件变量，在等待master响应时与mutex结合使用使线程阻塞
	uint8_t *obuff;//发送缓冲区，默认为4K大小，type+size+msgid+...
	uint32_t obuffsize;//发送缓冲区长度，默认4K
	uint32_t odataleng;//发送数据长度，type+size+msgid+...
	uint8_t *ibuff;//接收缓冲区，默认大小为64K大小，type+size+msgid+...
	uint32_t ibuffsize;//接收缓冲区长度，默认64K
	uint32_t idataleng;//接收的数据长度，size值

	uint8_t sent;		// packet was sent	消息请求包发送后置1
	uint8_t status;		// receive status	消息响应包中的status
	uint8_t rcvd;		// packet was received	消息响应包接收后置1
//	uint8_t waiting;	// thread is waiting for answer	已经发送了消息请求包后，正在等待接收消息响应包时置1，接收到消息请求包后清零
	uint8_t receiving;

	uint32_t rcvd_cmd;//消息响应包的消息类型type

	uint32_t packetid;// thread number 消息请求包和响应包中的msgid，依靠该值将消息请求和消息响应对应起来，每申请一个新的threc结构，packetid将在上一个threc结构的packetid基础上加1
	struct _threc *next;
} threc;
...
/**
 * 创建一个cmd命令的消息，返回threc变量的发送缓存区
 * size,指的运行这个程序后还需要添加的字符长度
 * */
uint8_t* fs_createpacket(threc *rec,uint32_t cmd,uint32_t size) {
	uint8_t *ptr;
	uint32_t hdrsize = size+4;//为什么+4？？？因为在hdrsize后加入了4字节的packetid，在master处对应于msfid
	pthread_mutex_lock(&(rec->mutex));	// make helgrind happy
	fs_output_buffer_init(rec,size+12);	//初始化一个消息的大小
	if (rec->obuff==NULL) {
		return NULL;
	}
	ptr = rec->obuff;
	//4*3=12字节
	//CLTOMA/MATOCL消息	从CLTOMA_SCLASS_CREATE (PROTO_BASE+350)开始，
	//					到MATOCL_FUSE_INVALIDATE_CHUNK_CACHE (PROTO_BASE+706)结束
	//					中间相差400，
...
//	uint32_t size = rec->size;
	...
	for (cnt=1 ; cnt<=maxretries ; cnt++) {
		...
		//fd是全局变量，在fs_connect中通过建立socket连接获取
		if (fd==-1) {
			pthread_mutex_unlock(&fdlock);
			usecto = 1000+((cnt<30)?((cnt-1)*300000):10000000);
			if (usectimeout>0) {
...
int fs_connect(uint8_t oninit,struct connect_args_t *cargs) {
	...
	do {
		...
		/**
		 * 如果有密码验证，首先需要发送一个CLTOMA_FUSE_REGISTER包。
		 * */
		if (havepassword) {
			wptr = regbuff;
			put32bit(&wptr,CLTOMA_FUSE_REGISTER);//65+8=73
			put32bit(&wptr,65);//写入消息长度  65=64+1 
			memcpy(wptr,FUSE_REGISTER_BLOB_ACL,64);
			wptr+=64;
			//get random，为了向maser获取一个32字节的随机数，用于与密码进行MD5运算加密，最后会将密码发送给master
			put8bit(&wptr,REGISTER_GETRANDOM);
			//8+65 8为CLTOMA_FUSE_REGISTER和消息长度
		...
		//发送一个CLTOMA_FUSE_REGISTER的包
		wptr = regbuff;
		put32bit(&wptr,CLTOMA_FUSE_REGISTER);
		put32bit(&wptr,rleng-8);//写入消息的长度（不包括头部8字节）
		memcpy(wptr,FUSE_REGISTER_BLOB_ACL,64);
		wptr+=64;
		//区分meta系统还是mfs系统
		...
		//如果有密码，把之前计算的16字节的数据填进去
		if (havepassword) {
			memcpy(wptr,digest,16);
		}
		//发送之前的REGISTER_NEWSESSION消息，如果rleng长度数据写失败——退出
...
// called before fork
/**
 * 与master进行首次连接和向master进行注册
 * 调用fs_connect函数进行实际连接与注册操作
 * */
...
uint8_t fs_writechunk(uint32_t inode,uint32_t indx,uint8_t chunkopflags,uint8_t *csdataver,uint64_t *length,uint64_t *chunkid,uint32_t *version,const uint8_t **csdata,uint32_t *csdatasize) {
	...
	*csdatasize = 0;
	...
	//建立一个发送给master的CLTOMA_FUSE_WRITE_CHUNK包，获取chunkserver的数据
	if (master_version()>=VERSION2INT(3,0,4)) {
