...
/**
 * 	从向FUSE接口注册的API函数来看，MooseFS实际实现了两个文件系统，
 *  一个是我们常用的MFS文件系统，可以进行各种标准的文件操作；
 *  另一个是META系统，主要用于回收站功能。
 * */
//实现META系统时需要向FUSE接口注册的一组API函数地址,用于回收站功能？
...
//实现MFS文件系统时需要向FUSE接口注册的一组API函数地址
...
//保存mfsmount时的传入参数
struct mfsopts {
	char *masterhost;	// master ip
	char *masterport;	// master port
	char *bindhost;		// 绑定ip，指定client端由哪个ip与master建立连接
	char *proxyhost;
	char *subfolder;	// 挂载到MFS文件系统子目录
	char *password;		// 向master认证时需要的密码
	char *passfile;
	char *md5pass;		// 向master认证时需要的密码直接以md5值方式给出
	char *preferedlabels;
	unsigned nofile;
	signed nice;		// mfsmount进程的优先级
	int mfssuid;
	int mfsdev;
	...
	int cachefiles;
	int keepcache;
	int passwordask;	// mfsmount时给出了-p参数要求用户输入密码
	int noxattrs;
	int noposixlocks;
	int nobsdlocks;
...
int mainloop(struct fuse_args *args,const char* mp,int mt,int fg) {
	...
	/**
	 * 调用fs_init_master_connection（参看5.1.4节）函数建立与master的连接并进行向其进行注册；
	 * */
...
/**
 * 如果mfsopts.memlock大于0，则调用mlockall将进程使用的全部的地址空间锁定在物理内存中，防止其被交换到swap空间
 * */
#ifdef MFS_USE_MEMLOCK
	if (mfsopts.memlock) {
...
//	dir_cache_init();
	//调用fs_init_threads函数，设置mastercomm.c全局变量maxretries为mfsopts.ioretries（I/O重试次数），
	//然后创建两个线程fs_receive_thread和fs_nop_thread；
	...
	if (mfsopts.meta) {
		...
		delay_init();
		//调用read_data_init函数，
		//设置readdata.c全局变量maxretries为mfsopts.ioretries（I/O重试次数）并创建一个线程read_data_delayed_ops
		...
		//调用write_data_init函数，设置writedata.c全局变量maxretries为mfsopts.ioretries（I/O重试次数），
		//根据mfsopts.writecachesize计算出cacheblocks用于写数据时缓存
...
//	dump_args("input_args",&args);
	...
	/**
	 * 设置mfsopts结构体变量的默认值
	 * 比如mfsopts.masterport字段需要填充master端口号，默认则设为9421；
	 * mfsopts.writecachesize字段的表示客户端写缓存大小，合理范围为16M~2048M等；
	 * */
	//主要解析master ip和port
	if (args.argc>1) {
		...
		//跳过"-o"及其参数后,查看有无"HOST[:PORT]:[PATH]"类型的参数
		if (optpos<args.argc) {
...
//	dump_args("changed_args",&args);
	...
	//cachefiles:preserve file data in cache
	...
	if (mfsopts.proxyhost==NULL) {//代理host？？
...
#define TIMEOUT_CLAMP(option,name,max) \
	...
	/**
	 * 调用make_fsname设置文件系统名称和类型。meta or mfs
	 */
	make_fsname(&args);
	if (mfsopts.mfssuid) {
...
//	dump_args("combined_args_before_fuse_parse_cmdline",&args);
	...
	/**
	 * 调用stats_term和strerr_term结束这些模块；
	 * */
	stats_term();
	strerr_term();
	//返回
