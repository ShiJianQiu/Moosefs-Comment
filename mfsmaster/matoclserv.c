...
void matoclserv_fuse_register(matoclserventry *eptr, const uint8_t *data, uint32_t length)
{
	...
	if (memcmp(data, FUSE_REGISTER_BLOB_ACL, 64) == 0)
	{
		...
		//在解析FUSE_REGISTER_BLOB_ACL后，还需要读取rcode,用来解析REGISTER消息类型，和master与chunkserve交互中的rversion类似，
		if (length < 65)
		{
		...
		case REGISTER_TOOLS:
			...
			sessionid = get32bit(&rptr);
			if (iptosesid_check(eptr->peerip))
			{ // patch for clients < 3.0
				eptr->mode = KILL;
				return;
			}
			...
			if (status == MFS_STATUS_OK)
			{
				...
				if (eptr->sesdata == NULL || sessions_get_peerip(eptr->sesdata) == 0)
				{ // no such session or session created by entries in metadata
					status = MFS_ERROR_BADSESSIONID;
				}
				else
				{
		...
		case REGISTER_CLOSESESSION:
			//client关闭连接时使用。client调用fs_nop_thread->{fs_send_amtime_inodes,fs_send_open_inodes,fs_close_session,tcpclose}
			if (length < 69){
...
void matoclserv_fuse_lookup(matoclserventry *eptr, const uint8_t *data, uint32_t length)
{
	...
	if (eptr->version >= VERSION2INT(3, 0, 40))
	{
		...
		if (status == MFS_STATUS_OK)
		{
			...
			count = 0;
			version = 0;
			if (filenode && (lflags & (LOOKUP_ACCESS_MODE_R | LOOKUP_ACCESS_MODE_W)) != 0)
...
void matoclserv_fuse_truncate(matoclserventry *eptr, const uint8_t *data, uint32_t length)
{
	...
	if (length <= 25)
	{
		...
		{
			if (uid == 0 && gid[0] != 0)
			{ // stupid "flags" patch for old clients
...
/**
 * 接收到来自Client的写消息请求——获取Chunkserver信息
 * */
...
void matoclserv_fuse_getfacl(matoclserventry *eptr, const uint8_t *data, uint32_t length)
{
	...
	msgid = get32bit(&data);//rec->packetid
	inode = get32bit(&data);
	acltype = get8bit(&data);
	if (length > 9)
	{ // just sanity length check
		if (length < 18)
		{
			syslog(LOG_NOTICE, "CLTOMA_FUSE_GETFACL - wrong size (%" PRIu32 ")", length);
			eptr->mode = KILL;
...
void matoclserv_gotpacket(matoclserventry *eptr, uint32_t type, const uint8_t *data, uint32_t length)
{
	...
	if (type == ANTOAN_UNKNOWN_COMMAND)
	{ // for future use
		return;
	}
	if (type == ANTOAN_BAD_COMMAND_SIZE)
	{ // for future use
		return;
	}
	//	printf("AQQ\n");
	if (eptr->registered == 0)
	{ // unregistered clients - beware that in this context sesdata is NULL
		switch (type)
		{
...
void matoclserv_read(matoclserventry *eptr, double now)
{
	...
	if (hup)
	{
		if (eptr->registered > 0 && eptr->registered < 100)
		{ // show this message only for standard, registered clients
...
void matoclserv_serve(struct pollfd *pdesc){
	...
	if (lsockpdescpos >= 0 && (pdesc[lsockpdescpos].revents & POLLIN))
	{
		...
		{
			//向eptr填充新的socket描述符做相关初始化
			tcpnonblock(ns);
			tcpnodelay(ns);
			eptr = malloc(sizeof(matoclserventry));
			passert(eptr);
			//将eptr连接到matoclservhead链表中
...
{
	//不同版本的host和port，MATOCL与老版本MATOCU
