...
static inline int32_t bio_internal_read(bio *b,uint8_t *buff,uint32_t leng) {
	//初始化时b->buff = malloc(buffersize); uint32_t buffersize=8;b->size = buffersize;b->leng = 0;b->pos = 0;b->type = 0;
	int32_t ret;
	if (b->type==0) {
...
static inline int bio_fill(bio *b) {
	////bio *b初始化时 b->leng = 0;b->pos = 0;
	//b->buff = malloc(buffersize); uint32_t buffersize=8;b->size = buffersize;
...
int64_t bio_read(bio *b,void *vdst,uint64_t len) {
	...
	//bio *b初始化时 b->leng = 0;b->pos = 0;此时len=8;b->size = buffersize很大的数;
