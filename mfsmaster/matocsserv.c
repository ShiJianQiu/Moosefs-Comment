...
enum
{
	RESERVE_BYTES,
	RESERVE_PERCENT,
	RESERVE_CHUNKSERVER_USED,
	RESERVE_CHUNKSERVER_TOTAL
};
...
typedef struct out_packetstruct
{
	...
	uint8_t *startptr;//指向data的指针
	uint32_t bytesleft;//data的长度
	uint8_t data[1];//数据
} out_packetstruct;
...
{
	uint8_t mode; //socket状态，HEADER表示即将接收header包，DATA表示即将接收DATA包，KILL表示即将删除该结构关闭此次连接
	int sock;
	int32_t pdescpos;
	...
	uint8_t hlstatus;	 // heavy load status (0 - unknown, 1 - ok, 2 - overloaded, 3 - int ernal rebalance is on)
	...
	uint32_t errorcounter; //matocsserv_error_occurred()
	uint16_t writecounter; //matocsserv_write_counters()
	...
	uint16_t csid; //csdata数组的index，一个副本对应一个csdata
	uint8_t registered;
	...
	uint32_t dist; //dist = totalspace/eptr->totalspace
		//eptr->dist = rndu32_ranged(dist*1000)/1000;<dist
	uint8_t first; //是不是第一次使用，matocsserv_serve初始化时first为1
	double corr;   //校正值，servers[i]->corr += expdist - (servers[i]->dist + i + 1 - cnt);
		//expdist = totalspace/ servers[i]->totalspace
		//0<=corr<=expdist+cnt-1

	struct csdbentry *csptr; //对应cs db entry

	struct matocsserventry *next;
} matocsserventry;

static matocsserventry *matocsservhead = NULL; //matocsserv_serve处初始化；头指针，指向matomlserventry结构的链表
static int lsock;
...
/**
 * 向日志输出matocsservhead信息，并对counter置0
 * */
...
uint16_t matocsserv_getservers_ordered(uint16_t[MAXCSCOUNT])
{
	//临时数据结构
	static struct servsort
	{
		double space;  //块服务器空间利用率（块服务器上已用空间占总空间的比例）
		uint16_t csid; //对应的csid
	} servtab[MAXCSCOUNT];
	matocsserventry *eptr;
	uint32_t i, scnt;
	...
	{
		//eptr->mode不是kill状态。有总空间且大于0。被使用空间小于总空间。有对应的cs db entry。且hlstatus是默认状态、ok或rebalance。csdb_server_is_being_maintained
...
{
	matocsserventry *eptr;
	double expdist; //expdist=totalspace/servers[i]->totalspace>1
	uint32_t i;
	uint64_t totalspace;
	static struct rservsort
	{
		double err;			  //负载均衡评价数
		matocsserventry *ptr; //指向matocsserventry* servers[]
	} servtab[MAXCSCOUNT];
	...
	for (i = 0; i < cnt; i++)
	{
		//如果还未使用，first=1，此时writecounter=0，wrepcounter=0
		if (servers[i]->first)
		{
			servtab[i].err = 1.0;
		}
		...
		{
			expdist = totalspace;
			expdist /= servers[i]->totalspace; // >1 ，可能很大
			servtab[i].err = (expdist + servers[i]->corr) / (servers[i]->dist + 1);
		}
		//writecounter被写的次数
		//wrepcounter，复制时，chunkserver作为目标正被写的次数
 ...
 * @msg: 在
 * @param 
 * {matocsserventry* servers[MAXCSCOUNT]} 
 * {uint32_t cnt:传入参数为c->allvalidcopies} 
 * @return: 
 */
static inline void matocsserv_weighted_roundrobin_used(matocsserventry *servers[MAXCSCOUNT], uint32_t cnt)
{
	...
	fcnt += cnt; //fcnt=c->allvalidcopies=servercnt
	if (fcnt > (totalcnt * 10))
	{ // correlation fixer   此时负载都很大,需要的chunk大于有效的10倍
		fcnt = 0;
		for (eptr = matocsservhead; eptr; eptr = eptr->next)
		{
			if (eptr->mode != KILL && eptr->totalspace > 0 && eptr->usedspace <= eptr->totalspace && eptr->csptr != NULL)
			{
				...
				//dist = totalspace/eptr->totalspace
				//生成一个与dist相关的随机数，小于totalspace/eptr->totalspace
	...
	{ // standard weighted round-robin
		for (i = 0; i < cnt; i++)
		{
			if (servers[i]->first)
			{
				//此时servers[i]->corr=0.0
				servers[i]->first = 0;
			}
			else
			{
				expdist = totalspace;
				expdist /= servers[i]->totalspace; //expdist = totalspace/ servers[i]->totalspace
				//此时servers[i]->dist=cnt
				//corr校正？初始为0 ,    0<=corr<=expdist+cnt-1
				//servers[i]->corr = （expdist - servers[i]->dist）+cnt-i-1     >0
				servers[i]->corr += expdist - (servers[i]->dist + i + 1 - cnt);
			}
			servers[i]->dist = cnt - i - 1; //0=<servers[i]->dist=<cnt-1
			servers[i]->create_total_counter++;
		}
	}
}
...
void matocsserv_getservers_test(uint16_t *stdcscnt, uint16_t stdcsids[MAXCSCOUNT], uint16_t *olcscnt, uint16_t olcsids[MAXCSCOUNT], uint16_t *allcscnt, uint16_t allcsids[MAXCSCOUNT])
{
	...
	*allcscnt = 0;
	...
	if ((gracecnt * 5) > (gracecnt + stdcnt))
	{ // there are more than 20% CS in 'grace' state - add all of them to the list
		while (gracecnt > 0)
		{
...
/**
 * 返回值是返回符合要求的数量，csids记录符合要求的csid，overloaded记录超载荷的数量
 * */
uint16_t matocsserv_getservers_wrandom(uint16_t csids[MAXCSCOUNT], uint16_t *overloaded)
{
	matocsserventry *servtab[MAXCSCOUNT]; //临时数据结构
	matocsserventry *eptr;
	uint32_t i;
	uint32_t gracecnt; //统计eptr->hlstatus=HLSTATUS_REBALANCE和HLSTATUS_GRACEFUL，即再平衡和优秀
	uint32_t stdcnt;   //std = standard servers，统计eptr->hlstatus=HLSTATUS_DEFAULT或eptr->hlstatus=HLSTATUS_OK
	uint32_t totalcnt;
	gracecnt = 0;
	stdcnt = 0; //standard servers count
	totalcnt = 0;
	*overloaded = 0;
	//遍历chunk server节点
	for (eptr = matocsservhead; eptr && totalcnt < MAXCSCOUNT; eptr = eptr->next)
	{
		...
		if (eptr->mode != KILL && eptr->totalspace > 0 && eptr->usedspace <= eptr->totalspace && eptr->csptr != NULL)
		{
			...
			*/
			if ((eptr->totalspace - eptr->usedspace) > (MFSCHUNKSIZE * (1U + eptr->writecounter * 10U)))
			{
				//heavy load status (0 - unknown, 1 - ok, 2 - overloaded, 3 - internal rebalance is on)
				//状态满足，不是heavy load状态
				if (eptr->hlstatus != HLSTATUS_OVERLOADED)
				{
					//符合条件，总数++
					totalcnt++;
					//状态是rebalance和graceful时，grace数++
					if ((eptr->hlstatus != HLSTATUS_DEFAULT && eptr->hlstatus != HLSTATUS_OK) || csdb_server_is_being_maintained(eptr->csptr))
					{
						gracecnt++;
						//从后往前存
	...
	//总共有5个状态
	//(gracecnt*5) > (gracecnt+stdcnt)  ==》 gracecnt*4 > stdcnt
	if ((gracecnt * 5) > (gracecnt + stdcnt))
	{ // there are more than 20% CS in 'grace' or 'rebalance' state - add all of them to the list
		//将满足gracecnt的serv添加到stdcnt之后
		while (gracecnt > 0)
		{
...
//chunk.c matocsserv_useservers_wrandom(chosen,c->allvalidcopies);
...
//代替matocsserv_getservers_lessrepl

/**
 * @name: matocsserv_get_server_groups
 * @test: test font
 * @msg: 
 * @param {uint16_t csids[MAXCSCOUNT],一个空的MAXCSCOUNT大小的uint16_t数组
 * double replimit,chunkserver作为replicate的源被写的最大次数
 * uint16_t positions[4]，每种chunk server status对应的个数，初始为空} 
 * @return: 
 */
void matocsserv_get_server_groups(uint16_t csids[MAXCSCOUNT], double replimit, uint16_t positions[4])
{
	...
	for (stage = 0; stage < 2; stage++)
	{
		...
		for (eptr = matocsservhead; eptr && i < MAXCSCOUNT; eptr = eptr->next)
		{
			...
			if (eptr->mode != KILL && eptr->csptr != NULL)
			{
				...
				{
					//不是HLSTATUS_DEFAULT且不是HLSTATUS_OK，或者不是csdb_server_is_being_maintained
	...
	{
		r = rndu32_ranged(i + 1); //生成一个小于i+1的随机数
		//交换i与r对应的csids
		if (r != i)
		{
			x = csids[i];
...
uint8_t *matocsserv_createpacket(matocsserventry *eptr, uint32_t type, uint32_t size)
{
	...
	psize = size + 8;
	//给outpaccket分配空间
	//offsetof(out_packetstruct, data) 返回data相对于out_packetstruct结构体的偏移量
	//这里psize相当于给data的开创的内存空间
	...
	outpacket->next = NULL;
	//将outpacket添加到eptr的outpacket链表中
...
void matocsserv_register(matocsserventry *eptr, const uint8_t *data, uint32_t length)
{
	...
	else
	{
		...
		else if (rversion == 50 || rversion == 60)
		{
			...
			if (eptr->version >= VERSION2INT(1, 6, 28))
			{ // if chunkserver version >= 1.6.28 then send back my version
				uint8_t *p;
				if (rversion == 50)
				{
...
{
	//type 不是以下四种状态中任何一种 且 eptr->csid 为初始化状态MAXCSCOUNT，会报Warning “unregistered chunk server”
	//即判断是否是注册状态
	...
	case CSTOMA_REPLICATE:
		//replication消息 在master指 导下完成chunkserver间的块拷贝过程。
		matocsserv_got_replicatechunk_status(eptr, data, length);
		break;
	case CSTOMA_DUPLICATE:
		//duplicate消息 用于在chunkserver上做一个chunk及其块文件的自拷贝。
	...
	case CSTOMA_TRUNCATE:
		//truncate消息 用于在chunkserver上对一个chunk的块文件做截断操作，即重新设置文件长度。
		matocsserv_got_truncatechunk_status(eptr, data, length);
		break;
	case CSTOMA_DUPTRUNC:
		//duptrunc消息 用于在chunkserver上先对一个源chunk及其块文件做自拷贝产生目标chunk，然后对目标chunk的块文件做截断操作，即重新设置文件长度。
...
void matocsserv_read(matocsserventry *eptr, double now)
{
	...
	for (;;)
	{
		/**从eptr->sock读取数据到readbuff中
		 * eptr->sock文件描述符
		 * readbuff + rbleng 读出的缓冲区
		 * readbuffsize - rbleng 每次去读的字节数
		 * 成功返回读取的字节数 i
		 * */
	...
	while (rbpos < rbleng)
	{
		//eptr->input_bytesleft在初始化时，为8
		if ((rbleng - rbpos) >= eptr->input_bytesleft)
		{
			//从readbuff读取8字节到eptr->input_startptr中
		...
		{
			/**
			 * 在初始化时eptr->input_startptr = eptr->input_hdr;
			 * 其中	uint8_t input_hdr[8];
			 * 		uint8_t *input_startptr;//指针
			 * 即input_startptr是指向input_hdr数组的指针
			 * 此时ptr也是指向input_hdr[8]数组的指针，
			 * 而input_hdr在上面的代码memcpy中得到了初始化
			 * */
			ptr = eptr->input_hdr;
			/**获得type类型与leng长度，type类似MATOCS_CREATE,leng为create_packet时的输入参数leng
			 * */
			type = get32bit(&ptr);
			leng = get32bit(&ptr);
			/**
			...
			//给input_packet分配空间
			//offsetof(in_packetstruct, data) 返回data相对于 in_packetstruct 结构体的偏移量
			//这里psize相当于给data的开创的内存空间
			...
			//input_startptr指向eptr->input_packet->data
...
void matocsserv_parse(matocsserventry *eptr)
{
	...
	{
		//循环遍历eptr中的inputhead链表，其中ipack->type, ipack->data在read函数中被解析出来
...
/**
 * 主要完成的工作是向一个pollfd类型的数组填充被监视的文件描述符fd和设置监控事件
 * */
...
void matocsserv_serve(struct pollfd *pdesc)
{
	...
	matocsserventry *eptr;
	int ns;
	static double lastaction = 0.0; //静态局部变量。
	...
	if (lastaction > 0.0)
	{
		...
		if (timeoutadd > 1.0)
		{ // more than one second passed - then fix 'timeout' timestamps
			for (eptr = matocsservhead; eptr; eptr = eptr->next)
			{
	...
	lastaction = now;
	//lsockpdescpos为matocsservhead末尾
	//有读请求POLLIN
	if (lsockpdescpos >= 0 && (pdesc[lsockpdescpos].revents & POLLIN))
	{
		...
		{
			//设置新socket连接为非阻塞非延时模式
			tcpnonblock(ns);
			tcpnodelay(ns);
			//分配一个matoxxserventry类型指针变量eptr
			eptr = malloc(sizeof(matocsserventry));
			passert(eptr);
			//将eptr连接到matoxxservhead链表中
	...
	for (eptr = matocsservhead; eptr; eptr = eptr->next)
	{
		...
		if (eptr->pdescpos >= 0)
		{
			...
			if ((pdesc[eptr->pdescpos].revents & (POLLERR | POLLIN)) == POLLIN && eptr->mode != KILL)
			{
				...
				//解析出inputpacket，其中type和leng在inputpacket中
	...
	for (eptr = matocsservhead; eptr; eptr = eptr->next)
	{
		...
		{
			//发送空包。该操作主要是为了保证client与master连接不会因为长时间空闲而断开。
...
/**
 * 释放资源，关闭tcp socket，关闭read buffer
 * */
...
int matocsserv_init(void)
{
	//加载配置参数
	matocsserv_reload_common();
	//获取监听端口 char *类型，这里可以是单个host、port，也可以是一串，在配置文件中读取，/mfsdata/
	ListenHost = cfg_getstr("MATOCS_LISTEN_HOST", "*");
	//这类DEFAULT_MASTER_CS_PORT变量，需要在configure.ac中配置
	ListenPort = cfg_getstr("MATOCS_LISTEN_PORT", DEFAULT_MASTER_CS_PORT);
	//初始化创建一个tcp socket，返回一个socket描述符
	lsock = tcpsocket();
	//socket建立失败时，返回值<0
	if (lsock < 0)
	{
	...
	//socket配置TCP_DEFER_ACCEPT模式，不释放端口，客户端会一直以为处于链接状态
