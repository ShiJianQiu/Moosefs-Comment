...
static double MaxWriteRepl[4];	//chunkserver作为replicate的源被写的最大次数
static double MaxReadRepl[4];	//chunkserver作为replicate的源被读的最大次数
...
static inline uint16_t chunk_creation_servers(uint16_t csids[MAXCSCOUNT],uint8_t sclassid,uint8_t *olflag) {
	...
	//找到符合要求的chunkserver，servcount记录符合要求的serv个数，csid记录chunkserver id，overloaded记录超载数
...
int chunk_univ_multi_modify(uint32_t ts,uint8_t mr,uint8_t continueop,uint64_t *nchunkid,uint64_t ochunkid,uint8_t sclassid,uint8_t *opflag) {
	...
	if (ochunkid==0) {	// new chunk
		if (mr==0) {
			//chunk_creation_servers 包含 matocsserv_getservers_wrandom 新建chunk时副本放置算法
			//csids chunk server id
			servcount = chunk_creation_servers(csids,sclassid,&overloaded);
			if (servcount==0) {
			...
			for (i=0 ; i<c->allvalidcopies ; i++) {
				s = slist_malloc();
				s->csid = csids[i];//副本存在不同的chunk server上
				s->valid = BUSY;
				s->version = c->version;
				//把slist存到chunk结构体中，一个chunk可以存多个副本，每个副本在不同的chunk server上，但是chunk id 相同
				s->next = c->slisthead;
				c->slisthead = s;
				chosen[i] = cstab[s->csid].ptr;
				stats_chunkops[CHUNK_OP_CREATE_TRY]++;
				//向chunkserver发送MATOCS_CREATE消息
...
//create new chunk or modify old chunk, inode to 64bit and use new chunkid system
...
//	static uint16_t bservcount;
	static uint16_t *rcsids = NULL;//一个MAXCSCOUNT大小的uint16_t数组
