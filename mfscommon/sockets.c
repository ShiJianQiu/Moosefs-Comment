...
static inline int sockaddrfill(struct sockaddr_in *sa,const char *hostname,const char *service,int family,int socktype,int passive) {
	 ...
	 * reshead：本函数通过reshead指针参数返回一个指向addrinfo结构体链表的指针。
	 * 返回值：0——成功，非0——出错
	 * */
...
/**
 * 控制socket接口的模式
 * ioctlsocket(sock, FIONBIO, &yes) 非阻塞设置，允许(1)或禁止(0)套接口s的非阻塞模式。
 * */
...
/**
 * 接受过滤器。服务器不等待最后的ACK包而仅仅等待携带数据负载的包。
 * 
 * 服务端处于监听时，客户端connect；
 * 服务端会收到syn包，并发送synack；
 * 当客户端收到synack并发送裸ack时，服务端accept创建一个新的句柄，这是不支持TCP_DEFER_ACCEPT选项下的流程。
 * 如果支持TCP_DEFER_ACCEPT，收到裸ack时，不会建立连接，操作系统不会Accept，也不会创建IO句柄。
 * 操作系统应该在若干秒后，会释放相关的链接；
 * 但没有同时关闭相应的端口，所以客户端会一直以为处于链接状态，
 * 如果Connect后面马上有后续的发送数据，那么服务器会调用Accept接收这个连接。
 * */
...
/**
 * tcp socket可重复绑定，端口释放后立即就可以被再次使用。
 * */
int tcpreuseaddr(int sock) {
	int yes=1;
	/**
	 * SO_REUSEADDR端口释放后可立即使用
