...
#define CONNECT_RETRIES 10	//connect retries
...
/**
 * 建立socket对象，设置无阻塞模式，connect建立连接，设置无延迟模式
 * */
...
uint8_t mainserv_read(int sock, const uint8_t *data, uint32_t length)
{
	...
	while (size > 0)
	{
		...
		if (((offset + size - 1) >> MFSBLOCKBITS) == blocknum)
		{ // last block
			blocksize = size;
		}
		else
		...
		if (pfd[1].revents & POLLIN)
		{
			...
			if (cmd == CSTOCL_WRITE_STATUS)
			{
				...
				else
				{
					...
					if (chunkid != wrjob->chunkid)
					{ // wrong chunkid
						pthread_mutex_unlock(&(wrdata.lock));
						break;
					}
					if (writeid != wrjob->writeid)
					{ // wrong writeid
						pthread_mutex_unlock(&(wrdata.lock));
						break;
					}
...
uint8_t mainserv_write(int sock, const uint8_t *data, uint32_t length)
{
	...
	//获取chunkid和version
	gchunkid = get64bit(&data);//8
	gversion = get32bit(&data);//4
	//如果需要转发
	if (length > ((protover) ? 13 : 12))
	{							   // write and forward data
		fwdip = get32bit(&data);   //4//多副本写时中转cs的下一级cs的ip
		fwdport = get16bit(&data); //2//多副本写时中转cs的下一级cs的port
		fwdsock = -1;
		//循环尝试CONNECT_RETRIES次连接
		for (i = 0; i < CONNECT_RETRIES && fwdsock < 0; i++)
		{
			...
			{	
				...
				if (protover)
				{
					memcpy(wptr, data, length - 13 - 6);//减去之前读取data的长度
				}
				else
				{
					memcpy(wptr, data, length - 12 - 6);//减去之前读取data的长度（没有protover）
				}
				//发送包，并且free packet
