...
typedef struct masterconn {
	...
	uint16_t masterport;
	uint16_t timeout;
	uint8_t masteraddrvalid;	//0: masterip&masterport is valid；1: invalid
...
static masterconn *masterconnsingleton=NULL;//用于维护连接
...
/**
 * 创建outpacket 返回指向outpacket->data的指针
 * */
...
void masterconn_sendregister(masterconn *eptr) {
	...
	if (eptr->new_register_mode) {
		...
		if (eptr->gotrndblob && AuthCode) {
			md5ctx md5c;
			buff = masterconn_create_attached_packet(eptr,CSTOMA_REGISTER,1+16+4+4+2+2+2+8+8+4+8+8+4);//leng=71
			put8bit(&buff,60);//rvesion
			md5_init(&md5c);
			...
			buff = masterconn_create_attached_packet(eptr,CSTOMA_REGISTER,1+4+4+2+2+2+8+8+4+8+8+4);//leng=55
			put8bit(&buff,60);
		}
...
#endif
		buff = masterconn_create_attached_packet(eptr,CSTOMA_REGISTER,1+4+4+2+2);//13
		put8bit(&buff,50);
		put32bit(&buff,VERSHEX);
		put32bit(&buff,myip);
		put16bit(&buff,myport);
...
/**
 * initialize the connection with the master
 * */
...
/**
 * 向一个pollfd类型的数组填充被监视的文件描述符fd和设置监控事件
 * ndesc 描述符个数
 * */
 ...
 * 对于本地socket描述符，如果有读请求，则建立一个新的socket描述符用于链接并加入到第二类描述符链表中
...
int masterconn_init(void) {
	...
	//每隔n秒向master发起一次连接尝试，默认5s；
	ReconnectionDelay = cfg_getuint32("MASTER_RECONNECTION_DELAY",5);
	//主节点master的IP；
	MasterHost = cfg_getstr("MASTER_HOST",DEFAULT_MASTERNAME);
	//主节点master的端口号,默认9420；
	MasterPort = cfg_getstr("MASTER_PORT",DEFAULT_MASTER_CS_PORT);
	//默认为*；
	BindHost = cfg_getstr("BIND_HOST","*");
	//超时时间，该时间内没有从master收到消息则断开与master的连接，默认60s；
	Timeout = cfg_getuint32("MASTER_TIMEOUT",0);
//	BackLogsNumber = cfg_getuint32("BACK_LOGS",50);
	...
	//3.0新功能 标识label。存储类允许您指定文件的副本应该存储在哪个Chunkservers上。
	masterconn_parselabels();
	//分配一个masterconn结构并赋值给全局变量指针masterconnsingleton
	eptr = masterconnsingleton = malloc(sizeof(masterconn));
	passert(eptr);

	eptr->masteraddrvalid = 0;//表示master的ip和port是正确的
	eptr->new_register_mode = 3;
	eptr->masterversion = 0;
	eptr->hlstatus = 0;
	eptr->mode = FREE;
...
//	logfd = NULL;
	...
	wantexittime = 0.0;
	//调用函数masterconn_initconnect尝试进行对master的初次连接；
	if (masterconn_initconnect(eptr)<0) {
		return -1;
	}
	//老版本此处会调用job_pool_new创建线程池，此版本在init.h的runTab中，通过initialize()初始化

	//注册各类函数
	main_time_register(REPORT_LOAD_FREQ,0,masterconn_reportload);
	//用于定期向master发送CSTOMA_SPACE消息
	main_time_register(REPORT_SPACE_FREQ,0,masterconn_check_hdd_space);
	//在产生错误时向master报告相关错误。
	main_eachloop_register(masterconn_check_hdd_reports);	
	//向timehead函数链表注册函数masterconn_reconnect用于每隔ReconnectionDelay秒（默认为5s）尝试连接master
	//实际该函数在masterconnsingleton->mode 为FREE情况下调用一次masterconn_initconnect函数；
	reconnect_hook = main_time_register(ReconnectionDelay,rndu32_ranged(ReconnectionDelay),masterconn_reconnect);
	//向dehead函数链表注册函数masterconn_term，用于mfschunkserver进程终止时释放一些变量占用的内存资源；
	main_destruct_register(masterconn_term);
	//向pollhead函数链表注册函数masterconn_desc与masterconn_serve用于与master进行消息通信。
	...
	//向rlhead函数链表注册函数masterconn_reload，用于接收SIGHUP信号后进行处理（比如重新加载配置文件）。
	//在该函数中实际工作仅仅是断开与master的连接，并没有重新加载配置文件信息。 
