...
void chartsdata_refresh(void) {
	...
	//memory usage (if available - rss + virt)
	if (mem_used(&rss,&virt)) {
		data[CHARTS_MEMORY_RSS] = rss;
		data[CHARTS_MEMORY_VIRT] = virt;
	}

	//traffic from master (bits/s)
	//traffic to master (bits/s) 
	...
	//number of low-level read operations per minute
	data[CHARTS_HDRLLOPR]=opr;
	//number of low-level write operations per minute
	...
	//number of chunk internal operations (duplicate,truncate,etc.) per minute 
